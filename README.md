...
pytest --cov                       # 133 offline deterministic tests (94.9% coverage)
...
| Serving & ops | `app.py`, `src/obs.py`, `Dockerfile`, `.github/workflows/ci.yml` | Versioned FastAPI (`/v1`), idempotency keys, size limits, request-ID JSON logging with per-stage latency, non-root Docker image, CI running lint + 133 offline tests (installed from the lockfile) + an end-to-end eval smoke run (configured; not yet executed on GitHub until first push). |

## Measured results — separately seeded regression/evaluation set

**The measured configuration is offline and deterministic — rules categoriser,
TF-IDF retrieval, template rationales, zero LLM calls.** The LLM adapters are
implemented and contract-tested with scripted fakes, but no live model was
evaluated; run `--provider ollama` to produce LLM-mode numbers with this same
harness. Eval set: 32 applicants / 4,567 transactions (seed 4242, jittered
parameters, generated separately from the 21-applicant dev set — but used during
development to verify label consistency, so it is a **regression set, not a
blind test set**).
...
| Transaction categorisation | **95.9%** (4,567 labelled, all 21 categories supported) | every error is an ambiguous merchant → `unknown` (abstention); **0** income-inflation errors; unknown rate 4.3%; 17 of 21 categories at 100% recall |
| Pipeline latency (in-process, not HTTP) | p50 ≈4 ms · p95 ≈7 ms | per stage (p50, ms): categorise ≈2 · affordability ≈0.9 · retrieve ≈0.7 · decide ≈0.03 — per-run figures in `reports/eval_summary.json` (timing fields vary run-to-run; metric values reproduce identically) |
...
decision), not live-LLM behaviour or real-world lending performance — the full
caveats are in [docs/EVALUATION.md](docs/EVALUATION.md). Retained evidence:
`reports/pytest_coverage.txt`, `reports/coverage.xml`,
`reports/docker_build_sandbox.log`.
...
tests/                  133 offline deterministic tests + 2 optional MiniLM tests (94.9% coverage)
docs/                   ARCHITECTURE · DECISIONS (ADRs) · EVALUATION
```

## Honest limitations

**The headline metrics above involve zero LLM calls** — they measure the
deterministic configuration; live-LLM categorisation accuracy, validation/repair
rates, injection behaviour, latency and cost are unmeasured until you run a
provider. No real data, no real Open Banking API, no real users, and never
deployed beyond local/container runs. Default model names (`llama3.2`,
`gpt-4o-mini`, `claude-3-5-haiku-latest`) are mutable aliases — pin exact
tags/versions when you run them. Storage is in-memory; there is no authentication; fairness
is **not** established (synthetic data cannot establish it); thresholds are
invented; the fictional policy corpus was not reviewed by a compliance
professional; and the reviewer flow is API-only (no UI). The measured headline
numbers are for the deterministic configuration — run `--provider ollama` for
live-model numbers. Each limitation and the path to production is discussed in
