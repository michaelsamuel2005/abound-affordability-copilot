...
A supervised classifier is the right *next* step once reviewer corrections accumulate labels — the review API already captures them.
...
Why: policy language and query language share vocabulary by construction, so lexical vectors already cover 40/40 runtime-decisive IDs at k=4 on the eval set; dense embeddings shine when wording diverges — supported but not required for the offline path.
Disadvantage: TF-IDF fails on paraphrase; two configurations to reason about.
Mitigation: same FAISS/cosine path for both; embedder name recorded in every result; eval reports per-embedder.
...
Chosen: 133 offline tests (plus 2 optional env-gated MiniLM integration tests), zero network; FakeLLM scripts exact model behaviour (malformed JSON, invented IDs, sign-inconsistent categories, sign-valid hostile patterns, mid-sequence outages).
...
Mitigation: documented gap; a scheduled live smoke-eval (same harness, `--provider ollama`) is the designed answer, not yet automated.
...
Disadvantage: image weight (~1 GB measured with a full-Python base) and a false air of deployability — a container is not a deployment.
